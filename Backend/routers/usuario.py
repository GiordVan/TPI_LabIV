...
def login(user: UsuarioLogin, db: Session = Depends(get_db)):
    ...
    if not bcrypt.checkpw(user.contrasenia.get_secret_value().encode('utf-8'), db_user.contrasenia.encode('utf-8')):
        raise HTTPException(status_code=401, detail="Contraseña incorrecta")

    # Crear token con los datos necesarios
    token_data = {
        "id": db_user.id,
        "email": db_user.email,
        "rol": db_user.rol
...
@usuario_router.post("/usuarios", tags=["Usuarios"], response_model=Usuario)
def register(user: UsuarioAuth, db: Session = Depends(get_db)):
    existing_user = db.query(UsuarioModel).filter(UsuarioModel.email == user.email).first()
    if existing_user:
...
@usuario_router.put("/usuarios/{usuario_id}", tags=["Usuarios"])
def update_usuario(usuario_id: int, user_update: UsuarioAuth, db: Session = Depends(get_db)):
    usuario = db.query(UsuarioModel).filter(UsuarioModel.id == usuario_id).first()
    ...
    hashed_password = bcrypt.hashpw(
        user_update.contrasenia.get_secret_value().encode('utf-8'),
        bcrypt.gensalt()
    ).decode('utf-8')
    ...
    return {"msg": "Usuario actualizado correctamente", "usuario": usuario}

@usuario_router.delete("/usuarios/{usuario_id}", tags=["Usuarios"])
